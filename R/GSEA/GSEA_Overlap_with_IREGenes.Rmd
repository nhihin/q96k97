...
```{r}
gs %>% arrange(fisher_p) %>% dplyr::select(geneset, contains("fdr"), n_with_ire, n) %>% head(50)
```

For genesets enriched in 3' IRE genes:
```{r}
gs %>% arrange(fisher_p_3) %>% dplyr::select(geneset, contains("fdr"), n_with_ire3, n) %>% head(50)
```

For genesets enriched in 5' IRE genes:
```{r}
gs %>% arrange(fisher_p_5) %>% dplyr::select(geneset, contains("fdr"), n_with_ire5, n) %>% head(50)
```
