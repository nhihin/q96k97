...
# gs %>% write.xlsx(here("R","GSEA","results","gs.xlsx"))

#gs <- readRDS(here("R","GSEA","results","gs.rds"))
```
...
gs %>%
  dplyr::select(geneset, contains("exp"), starts_with("obs"), )
```
...
```{r}
gs %>% arrange(fisher_p) %>% dplyr::select(geneset, contains("fdr"), n_with_ire, n, starts_with("obs_greater")) %>% head(50)
```

Sorted by genesets most enriched in 3' IRE genes:
```{r}
gs %>% arrange(fisher_p_3) %>% dplyr::select(geneset, contains("fdr"), n_with_ire3, n, starts_with("obs_greater")) %>% head(50)
```

Sorted by genesets most enriched in 5' IRE genes:
```{r}
gs %>% arrange(fisher_p_5) %>% dplyr::select(geneset, contains("fdr"), n_with_ire5, n, starts_with("obs_greater")) %>% head(50)
```
...
overlapDf <- gs %>% arrange(fisher_p) %>% dplyr::filter(fdr < 0.1 | fdr_3 < 0.1 | fdr_5 < 0.1) %>%
  ...
  bind_rows(data.frame(
    geneset = c("- GRCz11 Zebrafish Genes - All Predicted IREs", "- GRCz11 Zebrafish Genes - High Quality Predicted IREs"),
...
- [x] Stacked bar chart to indicate overlap of the top ~20 genesets. 
