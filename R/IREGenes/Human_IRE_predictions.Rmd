...
saveRDS(mayo_DGE, here("R/GSEA/data/human/mayo_DGE.rds"))
mayo_DGE <- readRDS(here::here("R/GSEA/data/human/mayo_DGE.rds"))
```
...
fit <- lmFit(voomData, design, block = mayo_DGE$samples$Name, correlation = dupcors) %>%
  ...
  eBayes(robust = TRUE)
fit <- readRDS(here("R/IREGenes/data/mayo/fit.rds"))
...
ireGenes<-readRDS(here::here("R/IREGenes/data/human_ireGenes.rds"))
...
h_idx <- limma::ids2indices(h_mapped, rownames(voomData))
```
...
gseaResults_humanHallmark <- combinedGSEA3(voomData, h_idx, design, contrasts, fit)

#saveRDS(gseaResults_humanHallmark, here("R/IREGenes/data/gseaResults_humanHallmark.rds"))
...
gseaResults_humanHallmark <- readRDS(here::here("R/IREGenes/data/gseaResults_humanHallmark.rds"))
```
...
## Visualisation of Gene Set Testing

- To visualise the results of the gene set testing, we will use back-to-back 
barplots as for the previous datasets.

### AD vs control in cerebellum compared to temporal cortex

- We would expect more severe changes in terms of gene expression in TC as 
that region is known to be more severely affected by AD. 

```{r fig.width=11}
# Calculate number of up and down genes from mroast proportions for 
# comparison 1
x_props <- gseaResults_humanHallmark$indivTest$Cerebellum_ADvsControl$mroast %>% 
  dplyr::select(Geneset, NGenes, PropUp, PropDown) %>% 
  dplyr::mutate(contrast="Cerebellum_ADvsControl",
                up = NGenes * PropUp,
...
# Calculate number of up and down genes from mroast proportions for 
# comparison 2
y_props <- gseaResults_humanHallmark$indivTest$TemporalCortex_ADvsControl$mroast %>% 
  dplyr::select(Geneset, NGenes, PropUp, PropDown) %>% 
  dplyr::mutate(contrast="TemporalCortex_ADvsControl",
                up = NGenes * PropUp,
                down = NGenes * PropDown,
                notSig = NGenes - up - down
  ) %>%
...
butterfly <- bind_rows(x_props, y_props) %>%
  ...
  dplyr::mutate(Geneset = factor(Geneset, levels = unique(Geneset)),
                ...
                contrast = factor(contrast, levels = c("Cerebellum_ADvsControl","TemporalCortex_ADvsControl"))) %>%
...
butterfly
#export::graph2ppt(butterfly, here("R/GSEA/fig/human_butterfly/ADvsControl"))
```

- Same but for the IRE gene sets

```{r fig.height = 5}
# Calculate number of up and down genes from mroast proportions for 
# comparison 1
x_props <- gseaResults_humanIREs$indivTest$Cerebellum_ADvsControl$mroast %>% 
  dplyr::select(Geneset, NGenes, PropUp, PropDown) %>% 
  dplyr::mutate(contrast="Cerebellum_ADvsControl",
                up = NGenes * PropUp,
                down = NGenes * PropDown,
...
# Calculate number of up and down genes from mroast proportions for 
# comparison 2
y_props <- gseaResults_humanIREs$indivTest$TemporalCortex_ADvsControl$mroast %>% 
  dplyr::select(Geneset, NGenes, PropUp, PropDown) %>% 
  dplyr::mutate(contrast="TemporalCortex_ADvsControl",
                up = NGenes * PropUp,
                down = NGenes * PropDown,
...
butterfly
export::graph2ppt(butterfly, here("R/GSEA/fig/human_butterfly/ADvsControl_IREs"))
```

- Include the following comparisons on the butterfly plot

    - AD vs control (cerebellum)
    - PSP vs control (cerebellum)
    - PA vs control
...
# Calculate number of up and down genes from mroast proportions for 
# comparison 2
y_props <- gseaResults_humanIREs$indivTest$Cerebellum_PAvsControl$mroast %>% 
  dplyr::select(Geneset, NGenes, PropUp, PropDown) %>% 
  dplyr::mutate(contrast="Cerebellum_PAvsControl",
                up = NGenes * PropUp,
                down = NGenes * PropDown,
                notSig = NGenes - up - down
  ) %>%
...
z_props <- gseaResults_humanIREs$indivTest$Cerebellum_PSPvsControl$mroast %>% 
  dplyr::select(Geneset, NGenes, PropUp, PropDown) %>% 
  dplyr::mutate(contrast="Cerebellum_PSPvsControl",
                up = NGenes * PropUp,
                down = NGenes * PropDown,
                notSig = NGenes - up - down
  ) %>%
...
butterfly <- bind_rows(x_props, y_props, z_props) %>%
  dplyr::mutate(Geneset = gsub(x = Geneset, pattern = "_", replacement = " "),
                Geneset = gsub(x = Geneset, pattern = "HALLMARK", replacement = "")) %>%
  reshape2::melt(id.vars=c("Geneset","NGenes","contrast")) %>%
  dplyr::arrange(desc(NGenes)) %>% 
  ...
  dplyr::mutate(Geneset = factor(Geneset, levels = unique(Geneset)),
                direction = factor(direction, levels = c("notSig", "down", "up")),
                contrast = factor(contrast, levels = c("Cerebellum_ADvsControl",
...
butterfly
export::graph2ppt(butterfly, here("R/GSEA/fig/human_butterfly/cerebellum_allVsControl"))
```

- Same but for Temporal Cortex

```{r}
x_props <- gseaResults_humanIREs$indivTest$TemporalCortex_ADvsControl$mroast %>% 
  dplyr::select(Geneset, NGenes, PropUp, PropDown) %>% 
  dplyr::mutate(contrast="TemporalCortex_ADvsControl",
                up = NGenes * PropUp,
                down = NGenes * PropDown,
...
# Calculate number of up and down genes from mroast proportions for 
# comparison 2
y_props <- gseaResults_humanIREs$indivTest$TemporalCortex_PAvsControl$mroast %>% 
  dplyr::select(Geneset, NGenes, PropUp, PropDown) %>% 
  dplyr::mutate(contrast="TemporalCortex_PAvsControl",
                up = NGenes * PropUp,
                down = NGenes * PropDown,
                notSig = NGenes - up - down
  ) %>%
...
z_props <- gseaResults_humanIREs$indivTest$TemporalCortex_PSPvsControl$mroast %>% 
  dplyr::select(Geneset, NGenes, PropUp, PropDown) %>% 
  dplyr::mutate(contrast="TemporalCortex_PSPvsControl",
                up = NGenes * PropUp,
                down = NGenes * PropDown,
                notSig = NGenes - up - down
  ) %>%
...
butterfly <- bind_rows(x_props, y_props, z_props) %>%
  ...
  dplyr::mutate(Geneset = factor(Geneset, levels = unique(Geneset)),
                direction = factor(direction, levels = c("notSig", "down", "up")),
                contrast = factor(contrast, levels = c("TemporalCortex_ADvsControl",
...
butterfly
export::graph2ppt(butterfly, here("R/GSEA/fig/human_butterfly/temporalcortex_allVsControl"))
```
...
# Calculate number of up and down genes from mroast proportions for 
# comparison 1
x_props <- gseaResults_humanHallmark$indivTest$Cerebellum_ADvsPA$mroast %>% 
  dplyr::select(Geneset, NGenes, PropUp, PropDown) %>% 
  dplyr::mutate(contrast="Cerebellum_ADvsPA",
                up = NGenes * PropUp,
                down = NGenes * PropDown,
                notSig = NGenes - up - down
  ) %>%
...
# Calculate number of up and down genes from mroast proportions for 
# comparison 2
y_props <- gseaResults_humanHallmark$indivTest$TemporalCortex_ADvsPA$mroast %>% 
  dplyr::select(Geneset, NGenes, PropUp, PropDown) %>% 
  dplyr::mutate(contrast="TemporalCortex_ADvsPA",
                up = NGenes * PropUp,
                down = NGenes * PropDown,
                notSig = NGenes - up - down
  ) %>%
...
butterfly <- bind_rows(x_props, y_props) %>%
  ...
  dplyr::mutate(Geneset = factor(Geneset, levels = unique(Geneset)),
                direction = factor(direction, levels = c("notSig", "down", "up")),
                contrast = factor(contrast, levels = c("Cerebellum_ADvsPA","TemporalCortex_ADvsPA"))) %>%
  ggplot(aes(x = Geneset, y = ngenes, fill = direction)) +
  geom_col(width=0.6) +
  ggpol::facet_share(~contrast, scales = "free") +
  scale_fill_manual(values=c("#DDDDDD","cornflowerblue","#FE4365"))+
...
cerebellum_upset_3 <- upset(l2m$ire3_all,
      ...
      keep.order = TRUE)
cerebellum_upset_3
export::graph2ppt(cerebellum_upset_3, here::here("R/GSEA/fig/human_upset/cerebellum_upset_3"))
```
...
cerebellum_upset_5 <- upset(l2m$ire5_all,
      ...
      keep.order = TRUE)
cerebellum_upset_5
export::graph2ppt(cerebellum_upset_5, here::here("R/GSEA/fig/human_upset/cerebellum_upset_5"))
```
...
export::graph2ppt(TemporalCortex_upset_3, here::here("R/GSEA/fig/human_upset/temporalCortex_upset_3"))
```
...
TemporalCortex_upset_5
 export::graph2ppt(TemporalCortex_upset_5, here::here("R/GSEA/fig/human_upset/temporalCortex_upset_5"))
```
